...
async function publishXHS(title, content, tags, imagePath) {
  ...
  let st = bc('state --format text').replace(/\n/g, ' ');
  let um = st.match(/\[(\d+)\][^\[]*button[^>]*>[^<]*上传图片/);
  if (!um) um = st.match(/\[(\d+)\]\s*<button[^>]*上传/);
  if (!um) um = st.match(/\[(\d+)\]\s*<div[^>]*上传/); // Win 备选
  if (!um) bail('找不到上传按钮');
  ...
  // Win: 检查是否需要回到发布页
  let postUrl = bc('eval "window.location.href"');
  if (postUrl.includes('from=tab_switch') || !postUrl.includes('/publish/publish')) {
    console.log('  🔄 页面跳转了，导航回发布页...');
    bc('navigate "' + CONFIG.publishUrl + '"');
    sleep(3000);
    bc('wait stable --timeout 30000');
  }

  // Step 5: 标题 (Win 适配版 — 多选择器)
  console.log('✏️  填写标题 (Win)...');
  const safeTitle = title.replace(/"/g, '\\"').replace(/'/g, "'\\''");
  // 直接用 eval 设置标题，不依赖 state 索引
  bc('eval "var i=document.querySelector(\'input[placeholder*=\\\"标题\\\"]\')||document.querySelector(\'[class*=\\\"title\\\"] input\')||document.querySelector(\'[class*=\\\"d-input\\\"]\');if(!i)i=document.querySelector(\'input[type=\\\"text\\\"]\');if(i){i.value=\'' + safeTitle.replace(/\\/g, '\\\\') + '\';i.dispatchEvent(new Event(\'input\',{bubbles:true}));i.dispatchEvent(new Event(\'change\',{bubbles:true}))}else{throw new Error(\'no title input\')}"');
  sleep(800);

  // Step 6: 正文 (Win 适配版)
  console.log('📝 填写正文 (Win)...');
  const cf = path.join(os.tmpdir(), 'xhs_body_' + Date.now() + '.txt');
  fs.writeFileSync(cf, content, 'utf8');
  const raw = fs.readFileSync(cf, 'utf8');
  bc('eval "var c=decodeURIComponent(\'' + encodeURIComponent(raw) + '\');var ed=document.querySelector(\'[class*=\\\"ql-editor\\\"]\')||document.querySelector(\'[contenteditable=\\\"true\\\"]\')||document.querySelector(\'.tiptap-container [contenteditable]\')||document.querySelector(\'[role=textbox][contenteditable]\');if(!ed)throw new Error(\'no editor\');ed.focus();if(ed.innerHTML){ed.innerHTML=\'\'}c.split(\'\\n\').forEach(function(l){var p=document.createElement(\'p\');p.textContent=l||\'\\u200B\';ed.appendChild(p)});ed.dispatchEvent(new Event(\'input\',{bubbles:true}))"');
  fs.unlinkSync(cf);
  sleep(1000);
